...
results_dir = DATA_DIR / "results/removal_runs_betweenness_weighted_node_BE"
results_dir.mkdir(parents=True, exist_ok=True)

attributes = load_gtfs(str(DATA_DIR / "sqlite/belgium.sqlite"))
L_graph = load_graph(DATA_DIR / "pkl/belgium_routesCleaned.pkl")
log_path = results_dir / "removal_betweenness_weighted_BE.log"
    ...
    try:
        ...
        original_efficiency, efficiencies, pct_remaining, removed_nodes, removal_times = \
            simulate_fixed_node_removal_efficiency(
                ...
                pct_to_remove=100,
                removal_type='node',
                method='betweenness_weighted',
                seed=seed
            )

        output_filename = f"betweenness_weighted_removal_seed{seed}_nodes{num_nodes_total}.csv"
